...
export async function requestPrivateInterpretation({chart,natalChart,houseLords,origin,destination,bearing,direction,distanceKm,selectedDate}){
  ...
  const response=await fetch('/api/interpret',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
    ...
    natalAsc:natalChart.asc,natalHouseCusps:natalChart.houseCusps
  })});
