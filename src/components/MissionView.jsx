...
import {computeChart,computeNatalChart,calculateHouseLords,buildJourneyReading,bearingBetween,destinationZoneFromBearing,distanceKmBetween,HOUSE_MEANINGS} from '../lib/astro';
...
export default function MissionView({mission,gps,gpsError,onBack}){
  ...
  const focusText=selected?privateReading?.planetPredictions?.[selected.id]:selectedHouse?privateReading?.housePredictions?.[selectedHouse]:null;
  ...
  return <div className={mapFullscreen?'mission-page map-is-fullscreen':'mission-page'}>
    ...
    {view==='compass'&&!mapFullscreen&&chart&&<div className="layout main"><section className="wheel-card"><ZodiacWheel chart={chart} planets={planets} selectedPlanet={selected} onSelectPlanet={choosePlanet} selectedHouse={selectedHouse} onSelectHouse={chooseHouse} destinationBearing={bearing}/></section><div className="stack"><LiveTrackingControls mode={trackingMode} onModeChange={changeTrackingMode} gps={gps} gpsError={gpsError} movedMeters={movedMeters} lastAnalysisAt={lastAnalysisAt}/><section className="card prediction-focus"><div className="section-title">{focusTitle}</div><p>{focusText||'Click any planetary glyph or numbered house sector on the wheel. The panel will show its forecast using natal contacts, current transit aspects, destination-zone activation and the private keyword layer.'}</p></section><OutcomeFeedback reading={reading}/></div></div>}
    {view==='map'&&<div className={mapFullscreen?'fullscreen-map-wrap':'mission-map-wrap'}><GoogleMissionMap location={liveLocation} analysisLocation={analysisLocation} destination={mission.destination} fullscreen={mapFullscreen} radiusMeters={wheelRadiusMeters} followUser={trackingMode!=='static'}/><MapScaleControls radiusMeters={wheelRadiusMeters} unit={distanceUnit} onUnitChange={setDistanceUnit} onRadiusChange={setWheelRadiusMeters}/><div className="map-tracking-overlay"><LiveTrackingControls mode={trackingMode} onModeChange={changeTrackingMode} gps={gps} gpsError={gpsError} movedMeters={movedMeters} lastAnalysisAt={lastAnalysisAt}/></div><button className="fullscreen-button" onClick={()=>setMapFullscreen(v=>!v)}>{mapFullscreen?<Minimize2 size={17}/>:<Maximize2 size={17}/>} {mapFullscreen?'Exit full screen':'Full screen'}</button>{chart&&<div className="map-wheel-overlay"><ZodiacWheel chart={chart} planets={planets} selectedPlanet={selected} onSelectPlanet={choosePlanet} selectedHouse={selectedHouse} onSelectHouse={chooseHouse} destinationBearing={bearing} compact overlay radiusMeters={wheelRadiusMeters} distanceUnit={distanceUnit}/></div>}</div>}
    {view==='map'&&!mapFullscreen&&<div className="layout main under-map"><section className="card prediction-focus"><div className="section-title">{focusTitle}</div><p>{focusText||'Click a planet glyph or a numbered house zone on the compass overlay to open its forecast.'}</p></section><OutcomeFeedback reading={reading}/></div>}
  </div>;
}
